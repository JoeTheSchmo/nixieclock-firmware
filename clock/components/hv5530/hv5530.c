...
volatile uint32_t hv5530_intensity = 0xB000;
...
//! Pointer to the registers to load
volatile uint8_t *hv5530_tx_regs = hv5530_xfade_new;
//! Current TX byte offset
...
void SPI_Handler() {
	...
	if (spi_is_tx_ready(SPI)) {
		if (hv5530_tx_pos < 8) {
			...
			spi_put(SPI, hv5530_tx_regs[hv5530_tx_pos++]);
		} else {
 ...
 * register is set determining the overall duty cycle for the display. Varying
 ...
 *
 * Counter A represents the cross-fade point. When a match occurs on counter A
 * the the new digits are turned off and the old digits are turned on.
 */
void TC0_Handler() {
	...
	if (status & TC_SR_CPAS) {
		// Check if we should cross-fade point or disable compare on RA
		ra = tc_read_ra(TC0, 0);

		if (ra < hv5530_intensity) {
			// Display the old state
			hv5530_tx_regs = hv5530_xfade_old;
			hv5530_write_registers();
		} else {
			tc_disable_interrupt(TC0, 0, TC_IDR_CPAS);
			ra = 0;
		}
	}
	...
	if (status & TC_SR_COVFS) {
		...
		if (ra > 0) {
			...
			tc_write_ra(TC0, 0, ra + 0x250);
		}

		// Change the intensity where required
		if (rc < hv5530_intensity) {
			if (hv5530_intensity - rc > 0x40) {
				tc_write_rc(TC0, 0, rc + 0x40);
			} else {
				tc_write_rc(TC0, 0, hv5530_intensity);
			}
		} else if (rc > hv5530_intensity) {
			if (rc - hv5530_intensity > 0x40) {
				tc_write_rc(TC0, 0, rc - 0x40);
			} else {
				tc_write_rc(TC0, 0, hv5530_intensity);
			}
 ...
 *
 * This function converts a time of day to a register map and creates
 * a set of fade masks
 *
...
void hv5530_init(void) {
	...
	// Start with a predefined duty cycle
	tc_write_rc(TC0, 0, hv5530_intensity);
