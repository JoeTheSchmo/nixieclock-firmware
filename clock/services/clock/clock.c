...
void clock_refresh_globals(void) {
	...
	rtc_get_date(RTC, &(clock.year), &(clock.month), &(clock.date), &(clock.day));

	// read the time from the rtc
	rtc_get_time(RTC, &(clock.hour), &(clock.minute), &(clock.second));
}
...
int32_t clock_set_rtc_from_ds3231(void) {
	...
	if (RTC->RTC_VER & (RTC_VER_NVTIM | RTC_VER_NVCAL)) {
		return -1;
	}
...
void clock_init(void) {
	...
	// Enable RTC Interrupt (Priority 8)
	irq_register_handler(ID_RTC, 8);
